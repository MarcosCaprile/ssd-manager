...
void main() {
  ...
  test('booking window includes today and the next 13 days', () {
    ...
    expect(DutyRules.isWithinUpcomingWindow(now, DateTime(2026, 7, 17)), isTrue);
    expect(DutyRules.isWithinUpcomingWindow(now, DateTime(2026, 7, 30)), isTrue);
    expect(DutyRules.isWithinUpcomingWindow(now, DateTime(2026, 7, 31)), isFalse);
    expect(DutyRules.isWithinUpcomingWindow(now, DateTime(2026, 7, 16)), isFalse);
  });
  ...
  test('regular cancellation requires at least 48 hours', () {
    ...
    expect(DutyRules.canCancelRegularly(now.add(const Duration(minutes: 1)), DateTime(2026, 7, 19)), isFalse);
  });
